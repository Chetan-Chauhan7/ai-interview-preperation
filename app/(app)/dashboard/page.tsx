...
export default function DashboardPage() {
  const { recentSessions, recentAnalyses } = useAppStore();
            ...
            <CardContent className="space-y-2">
              {recentSessions.length === 0 ? (
                ...
                recentSessions.slice(0, 5).map((session) => (
                  <div key={session.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent cursor-pointer transition-colors">
                    ...
                    <Badge variant="secondary" className="text-xs flex-shrink-0">
                      {session.session_type === 'jd' ? 'JD' : 'Role'}
                    </Badge>
                  </div>
                ))
              )}
            ...
            <CardContent className="space-y-2">
              {recentAnalyses.length === 0 ? (
                ...
                recentAnalyses.slice(0, 5).map((analysis) => (
                  <div key={analysis.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent transition-colors">
                    ...
                    <div className={`text-sm font-bold ${
                      analysis.ats_score >= 80 ? 'text-emerald-500' :
                      analysis.ats_score >= 60 ? 'text-amber-500' : 'text-red-500'
                    }`}>
                      {analysis.ats_score}%
                    </div>
                  </div>
